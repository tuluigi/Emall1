...
+ (NSString *)durationDataStingWithVideoSeconds:(CGFloat)videoSeconds{
    ...
    NSString *timeStr=@"";
    if (hour) {
        timeStr=[NSString stringWithFormat:@"%ld时",hour];
    }
    if (minute) {
        timeStr=[timeStr stringByAppendingString:[NSString stringWithFormat:@"%ld分",minute]];
    }
    if (second) {
        timeStr=[timeStr stringByAppendingString:[NSString stringWithFormat:@"%ld秒",second]];
    }
    return timeStr;
}
...
+ (NSString *)tenThousandUnitString:(NSInteger)numberValue{
    ...
    if (numberValue<unit) {//小于1w 显示实际数字
        str=[NSString stringWithFormat:@"%ld",numberValue];
    }else if(numberValue>(10*unit)){//大于10W 只显示整数万
      str=[NSString stringWithFormat:@"%ld万",(NSInteger)(numberValue/unit)];
